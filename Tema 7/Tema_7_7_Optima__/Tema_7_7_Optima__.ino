...
void loop(){
  ...
  unsigned int tiempo_mapeado;
  

  

  for ( int cuenta_atras = 9; cuenta_atras >= 0; cuenta_atras--){
    // Leemos el valor del potenciometro y lo guardamos
    valor_potenciometro = analogRead(A0);
  
    // Realizamos la conversion para al tiempo adecuado
    tiempo_mapeado = map(valor_potenciometro, 0, 1023, 100, 5000);
    
    // Mostramos en el display la cuenta atras    
    imprime_7_segmentos(cuenta_atras);
    delay(tiempo_mapeado);
  }
}
